...
export async function GET(req: Request) {
  ...
  if (wardNo) {
    ...
    const [ward, infra, potholes, crashes, air, spend, workOrders] = await Promise.all([
      ...
      supabase.from("bbmp_work_orders").select("work_order_id, description, contractor_name, contractor_phone, sanctioned_amount, net_paid, deduction, fy").eq("ward_no", wn).order("sanctioned_amount", { ascending: false, nullsFirst: false }).order("net_paid", { ascending: false, nullsFirst: false }).limit(50),
    ])
