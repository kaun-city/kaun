...
export async function GET(req: Request) {
  ...
  if (wardNo) {
    ...
    const [ward, infra, potholes, crashes, air, spend, workOrders] = await Promise.all([
      ...
      // Work orders join on bbmp_ward_no (DataMeet-243, backfilled from the
      // Kaun ward crosswalk). Defensive fallback to raw BBMP-Final-225 ward_no
      // for not-yet-backfilled rows → safe to deploy before/after backfill.
      supabase.from("bbmp_work_orders").select("work_order_id, description, contractor_name, contractor_phone, sanctioned_amount, net_paid, deduction, fy, contractor_code, division, budget_head, start_date, end_date, order_ref, sbr_ref, bill_ref, payment_status, data_source, ifms_wbid").or(`bbmp_ward_no.eq.${wn},and(bbmp_ward_no.is.null,ward_no.eq.${wn})`).order("sanctioned_amount", { ascending: false, nullsFirst: false }).order("net_paid", { ascending: false, nullsFirst: false }).limit(50),
    ])
